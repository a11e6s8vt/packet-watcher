...
/// This crate has a runtime dependency on artifacts produced by the `packet-watcher-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "packet-watcher-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `packet-watcher-ebpf`, so cargo will automatically rebuild us
        // if `packet-watcher-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `packet-watcher-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("packet-watcher-ebpf");
